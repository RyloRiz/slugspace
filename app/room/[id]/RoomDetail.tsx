...
function RoomNoteEditor({ roomId }: { roomId: number }) {
  const { getNote, setNote } = useRoomNotes();
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState("");
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const currentNote = getNote(roomId);
  ...
  if (!editing && !currentNote) {
    return (
      <button
        ...
        className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs text-muted hover:text-foreground hover:bg-surface dark:hover:bg-surface-dark border border-dashed border-border dark:border-border-dark hover:border-foreground/20 transition-all cursor-pointer w-full sm:w-auto"
      >
        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
  ...
  if (editing) {
    return (
      <div className="rounded-2xl border border-primary/20 dark:border-secondary/20 bg-card dark:bg-card-dark p-4 space-y-3">
        <div className="flex items-center gap-2">
          <svg className="w-3.5 h-3.5 text-primary dark:text-secondary" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
          </svg>
          <span className="text-[10px] font-bold text-muted uppercase tracking-widest">Personal Note</span>
        </div>
        <textarea
          ref={textareaRef}
          value={draft}
          ...
          placeholder="e.g., Outlet near the window, quiet floor, best whiteboard..."
          maxLength={200}
          rows={2}
          className="w-full px-3 py-2 rounded-xl text-sm bg-surface dark:bg-surface-dark border border-border dark:border-border-dark text-foreground placeholder:text-muted/40 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/30 resize-none"
        />
        <div className="flex items-center justify-between">
          <span className="text-[10px] text-muted">{draft.length}/200</span>
          <div className="flex items-center gap-2">
            <button onClick={cancel} className="px-3 py-1.5 rounded-lg text-xs text-muted hover:text-foreground transition-colors cursor-pointer">
              Cancel
            </button>
            <button onClick={save} className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-primary dark:bg-secondary text-white hover:bg-primary/90 dark:hover:bg-secondary/90 transition-colors cursor-pointer">
              Save
            </button>
          </div>
        </div>
  ...
  return (
    <div className="rounded-2xl border border-primary/10 dark:border-secondary/10 bg-primary/3 dark:bg-secondary/3 p-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-2.5 min-w-0">
          <svg className="w-3.5 h-3.5 text-primary/40 dark:text-secondary/40 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
          </svg>
          <p className="text-xs text-muted leading-relaxed">{currentNote}</p>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            onClick={startEditing}
            className="p-1.5 rounded-lg text-muted/40 hover:text-primary dark:hover:text-secondary hover:bg-primary/5 dark:hover:bg-secondary/5 transition-colors cursor-pointer"
            title="Edit note"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
          ...
          <button
            onClick={() => setNote(roomId, "")}
            className="p-1.5 rounded-lg text-muted/40 hover:text-booked hover:bg-booked/5 transition-colors cursor-pointer"
            title="Delete note"
          >
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
